		...
		<div class="row ml-0 mr-0">
			<div class="sign_left signin_left">
				...
				if ( !empty($top_ornament['id']) ) {
					...
					<img src="<?php echo HOVARD_DIR_IMG ?>/sign-up/top_ornamate.png" class="position-absolute top" alt="<?php esc_attr_e('top ornament', 'hovard'); ?>">
					<?php
				}
				if ( !empty($bottom_ornament['id']) ) {
					echo wp_get_attachment_image($bottom_ornament['id'], 'full');
				} else { ?>
					<img src="<?php echo HOVARD_DIR_IMG ?>/sign-up/bottom_ornamate.png" class="position-absolute bottom" alt="<?php esc_attr_e('bottom ornament', 'hovard'); ?>">
					<?php
				}
				if ( !empty($featured_image['id']) ) {
					echo wp_get_attachment_image($featured_image['id'], 'full');
				} else { ?>
					<img src="<?php echo HOVARD_DIR_IMG ?>/sign-up/door.png" class="position-absolute middle wow fadeInRight" alt="<?php esc_attr_e('door', 'hovard'); ?>">
					<?php
				}
				?>
					...
					if ( !is_user_logged_in() ) :
						...
						<form action="<?php echo esc_url(home_url( '/')); ?>wp-login.php" class="row login_form" method="post">
							<div class="col-lg-12 form-group">
								...
								<input type="text" class="form-control" id="username" name="log" placeholder="<?php esc_attr_e('Enter the username here.', 'hovard'); ?>">
							</div>
							<div class="col-lg-12 form-group">
								<label for="pwd" class="small_text"> <?php esc_html_e( 'Password', 'hovard' ); ?> </label>
								<div class="confirm_password">
									...
									<a href="<?php echo esc_url(home_url( '/')) . '/wp-login.php?action=lostpassword'; ?>" class="forget_btn">
										<?php esc_html_e( 'Forgotten password?', 'hovard' ); ?>
									</a>
								</div>
							</div>
							<div class="col-lg-12 text-center">
								<button type="submit" class="btn action_btn thm_btn">
									<?php echo !empty($submit_btn_label) ? esc_html($submit_btn_label) : esc_html__('Sign in', 'hovard'); ?>
								</button>
							</div>
						</form>
					<?php else : ?>
						<div class="text-center">
							<h2 class="signup_title">
								<?php esc_html_e('Welcome ', 'hovard'); echo esc_html($current_user->display_name); ?>
							</h2>
							<br>
							<p> <?php esc_html_e('You are logged in', 'hovard'); ?> </p>
						</div>
					<?php endif; ?>
				</div>
			</div>
